    ...
    return (
        <header className="z-[999] relative">
          ...
          py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
            {/*ul will hold all of the navigation bar links. name and hash.*/}
            {/*below we are importing the nav bar links from our data page*/}
            ...
            sm:flex-nowrap sm:gap-5">
              ...
              {/*below we are adding motion to the li's to animate the nav bar hover effect*/}
                ...
                {links.map((link) => (
                  ...
                  >
                  <Link 
                    className={clsx("flex w-full items-center justify-center px-3 py-3 hover:text-blue-500 transition dark:text-white dark:hover:text-blue-200", 
                      {"text-blue-500 dark:text-blue-300": activeSection === link.name,}
                    )}
                    href={link.hash}
                    onClick={() => { 
                ...
                >
...

                      {
                        link.name === activeSection && (
                          <motion.span className="bg-blue-100 rounded-full absolute inset-0 -z-10 dark:bg-blue-950"
